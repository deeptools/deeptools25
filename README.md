...
- `bench.snk` - Snakemake workflow for running benchmarks on different datasets. We are using `snakemake==8.24.1`. Downloads data, runs rust and legacy deeptools, compares outputs, plots average memory used and time. **Run with `--benchmark-extended` flag for more detailed profiling.**
