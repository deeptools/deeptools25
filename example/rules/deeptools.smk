rule multibamsummary:
    input:
        ...
        bamfiles = lambda wildcards: expand('deeptools_input/{sample}.bam', sample=[i for i in SAMPLES if wildcards.chip in i])
    output:
        npz = 'regions/{chip}_mbs.npz'
    threads: 10
    shell:'''
...
rule parse_de:
    input:
        ...
        up = 'regions/de_up.tsv',
        nonde = 'regions/nonde.tsv',
        mbs = 'regions/{chip}_mbs.npz',
        urop = 'regions/{chip}_uropa_finalhits.txt',
    output:
        down = 'deeptools_input/downreg_{chip}.bed',
        up = 'deeptools_input/upreg_{chip}.bed',
        nonde = 'deeptools_input/nonreg_{chip}.bed'
    params:
        chip = lambda wildcards: wildcards.chip,
        l2fc = config['l2fc']
    script:
        'scripts/parse_DE.py'
