...
targets = [(f["key"], f["links"]["self"]) for f in r["files"] if 'mouse.fna.gz' in f["key"] or 'mouse.gtf.gz' in f["key"]]
if not only_fna:
    targets += [(f["key"], f["links"]["self"]) for f in r["files"] if 'example' in f["key"]]
